...
--- Creates a default clickable button.
--- @param hint_ string: The hint (hover) text for the button.
--- @param device_ any
--- @param command_ any
--- @param arg_ any
--- @param arg_val_ any
...
function Mfd_button(hint_, device_, command_, arg_, arg_val_, arg_lim_, sound_) -- nytt
	local arg_val_ = arg_val_ or 1
	local arg_lim_ = arg_lim_ or {0, 1}
	...
	return {
		...
		arg_value = {arg_val_},
		arg_lim = {arg_lim_},
		use_release_message = {false}, -- changed from true to avoid double function call
		sound = sound_ and {{sound_}, {sound_}} or nil
	}
end

function Toggle_Button(hint_, device_, command_, arg_, sound_, animation_speed_)
...
function springloaded_2_position_tumb(hint_, device_, command_, arg_, sound_, animation_speed_)
	local animation_speed_ = animation_speed_ or anim_speed_default
	return {
		class               = {class_type.BTN}, -- Use BTN for spring-loaded
		hint                = hint_,
		device              = device_,
		action              = {command_},
		stop_action         = {command_}, -- Returns to default position
		arg                 = {arg_},
		arg_value           = {1},      -- Activates to position 1
		arg_lim             = {{0, 1}}, -- Two positions: 0 and 1
		updatable           = true,
		use_OBB             = true,
		use_release_message = {true}, -- Critical for spring-loaded behavior
		animated            = {true},
		animation_speed     = {animation_speed_},
		sound               = sound_ and {{sound_}} or nil
	}
end
